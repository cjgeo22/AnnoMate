...
class MaskingTool:
    ...
    def _build_ui(self):
        ...
        try:
            self.root.winfo_toplevel().configure(menu=self.menu)
        except Exception:
            pass
...
class InferenceTool:
    ...
    def _build_ui(self):
        ...
        for i, title in enumerate(("Original", "Heatmap", "Overlay", "Mask Overlay")):
            c = tk.Canvas(all_frame, bg="black")
            c.grid(row=0, column=i, sticky="nsew")
            all_frame.columnconfigure(i, weight=1)
            self.all_c[title] = c
        ...
        for c in list(self.canvases.values()) + list(self.all_c.values()):
            ...
            for ev in ("<MouseWheel>", "<Button-4>", "<Button-5>"):
                c.bind(ev, on_zoom)
            # pan right‐click
            c.bind("<ButtonPress-3>", on_pan_start)
            c.bind("<B3-Motion>", on_pan_move)
